...
- Lets the user choose a result language; OpenAI translates coordinator-facing result text after ranking while preserving hospital names, contacts, IDs, URLs, pin codes, and coordinates.
...
- Non-English result display is a presentation layer. Scores, distances, facility names, contacts, and links remain unchanged; translated evidence should still be verified against the source record before referral.
