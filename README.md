...
- Uses a global language selector in the app header. OpenAI translates the app chrome, search labels, placeholders, shortlist, chat controls, result cards, and map/result labels while preserving hospital names, contacts, IDs, URLs, pin codes, coordinates, and scores. Changing the language after a search reruns the result rendering in the selected language.
