...
- Displays an interactive referral map with the search origin and ranked facility markers.
...
- SQL warehouse resource with key `sql_warehouse`.
- Secret resource with key `openai_api_key`, containing your OpenAI API key.
- Unity Catalog table permissions for the app service principal.

The app expects these tables by default:

- `main.databricks_virtue_foundation_dataset_dais_2026.facilities`
- `main.databricks_virtue_foundation_dataset_dais_2026.india_post_pincode_directory`
- `main.databricks_virtue_foundation_dataset_dais_2026.nfhs_5_district_health_indicators`

If your catalog is not `main`, edit `DATABRICKS_CATALOG` in `app.yaml`.
...
Do not hardcode API keys in this repo or `app.yaml`. The app reads `OPENAI_API_KEY` from a Databricks secret resource.
...
- The current ranking is deterministic and evidence-grounded. OpenAI only parses the query.
- Web resolution is used for geography gaps, not to invent facility evidence.
- The map uses Plotly geo markers, so it does not require a Mapbox token.
