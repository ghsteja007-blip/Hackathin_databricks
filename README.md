...
- Lets the user choose a result language; OpenAI translates coordinator-facing result text and result-card labels after ranking while preserving hospital names, contacts, IDs, URLs, pin codes, coordinates, and scores.
...
- Non-English result display is a presentation layer. Scores, distances, facility names, contacts, and links remain unchanged; translated evidence should still be verified against the source record before referral. If translation is unavailable, the app falls back silently to dataset text rather than showing operational debug notes in the UI.
